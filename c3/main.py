...
def evaluate(request: ActionRequest):
    ...
    # Integrity and attestation passed. The operator still has the final
    # call: execution only happens on an explicit ALLOW. No response within
    # the timeout, or an explicit BLOCK, both stop the action - attestation
    # proves the framework wasn't tampered with, but it is not itself
    # authorization to act.
    operator_choice = alert_log.prompt_operator(action, target, reasoning)
    final_decision = operator_choice or "BLOCK"
    reason = (
        f"operator chose {operator_choice}"
        if operator_choice
        else "attestation succeeded, but no operator response within timeout, defaulted to BLOCK"
    )
