...
def _get_conn():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS baseline (path TEXT PRIMARY KEY, sha256 TEXT)")
    return conn
...
def reinitialize():
    ...
    conn.executemany("INSERT INTO baseline (path, sha256) VALUES (?, ?)", list(measurements.items()))
    conn.commit()
    conn.close()
    return measurements
